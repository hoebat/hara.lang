...
(impl/build-impl {:get-entry    (fn [context id]
                                  (get-in context [:entries id]))
                  :get-deps     (fn [context id]
                                  (get-in context [:graph id]))
                  :list-entries (fn [context]
                                  (keys (:entries context)))
                  :add-entry    (fn [context id entry deps]
                                  (-> context
                                      (assoc-in [:entries id] entry)
                                      (assoc-in [:graph id] (set deps))))
                  :remove-entry (fn [context id]
                                  (-> context
                                      (update :entries dissoc id)
                                      (update :graph dissoc id)))
                  :refresh-entry (fn [context id]
                                   context)
                  :step-construct (fn [context acc id]
                                    (conj acc (protocol.deps/-get-entry context id)))
                  :init-construct (fn [context]
                                    [])
                  :step-deconstruct (fn [context acc id]
                                      (vec (remove #(= id (:id %)) acc)))}
                 protocol.deps/IDeps
