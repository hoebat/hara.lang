...
public class Zip {
    ...
    public static Zipper zipper(Object root, IZipContext context, IZipHandler handler) {
        return new Zipper(null, List.Standard.EMPTY, (List) List.Standard.EMPTY.conj(context.wrapData(root)), 0, false, context, handler);
    }
    ...
    public static Zipper stepLeft(Zipper zipper) {
        if (zipper.atLeftMost()) {
            ...
            return new Zipper(zipper.parent, newLeft, newRight, zipper.depth, zipper.changed, zipper.context, zipper.handler);
        }
    }
    ...
    public static Zipper stepInside(Zipper zipper) {
        if (zipper.atInsideMost()) {
            ...
            return new Zipper(zipper, List.Standard.EMPTY, children, zipper.depth + 1, zipper.changed, zipper.context, zipper.handler);
        }
    }
    ...
    public static Zipper stepInsideLeft(Zipper zipper) {
        if (zipper.atInsideMostLeft()) {
            ...
            Object elem = zipper.left.peekFirst();
            List children = zipper.context.listElements(elem);
            Zipper parent = new Zipper(zipper.parent, (List) zipper.left.popFirst(), (List) zipper.right.cons(elem), zipper.depth, zipper.changed, zipper.context, zipper.handler);
            return new Zipper(parent, reverse(children), List.Standard.EMPTY, zipper.depth + 1, zipper.changed, zipper.context, zipper.handler);
        }
    }
    ...
    public static Zipper stepOutside(Zipper zipper) {
        if (zipper.atOutsideMost()) {
            ...
            if (zipper.changed) {
                ...
                return new Zipper(parent.parent, parent.left, (List) ((List) parent.right.popFirst()).cons(newNode), parent.depth, true, parent.context, parent.handler);
            } else {
                return parent;
            }
        }
    ...
    public static Zipper insertLeft(Zipper zipper, Object data) {
        ...
        List newLeft = (List) zipper.left.cons(elem);
        return new Zipper(zipper.parent, newLeft, zipper.right, zipper.depth, true, zipper.context, zipper.handler);
    }

    public static Zipper insertAndFocus(Zipper zipper, Object data) {
        Object elem = zipper.context.createElement(data);
        List newRight = (List) zipper.right.cons(elem);
        return new Zipper(zipper.parent, zipper.left, newRight, zipper.depth, true, zipper.context, zipper.handler);
    }
    ...
    public static Zipper find(Zipper zipper, java.util.function.Function<Zipper, Zipper> move, java.util.function.Predicate<Zipper> pred) {
    ...
    public static Zipper findLeft(Zipper zipper, java.util.function.Predicate<Zipper> pred) {
        IZipHandler handler = new BaseHandler() {
            @Override
            public Zipper onStepAtLeftMost(Zipper z) {
                return null;
            }
        };
        Zipper z = new Zipper(zipper.parent, zipper.left, zipper.right, zipper.depth, zipper.changed, zipper.context, handler);
