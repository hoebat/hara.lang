...
public interface Fn {
	...
	public interface T {
		...
		public class FnExact<R> implements I.Fn<R, Object, Object> {
			...
			public Function<Object[], R> getArgN() {
				return (args) -> {
					if(args.length != _num) {
						throw new Ex.Arity(args.length, "Only " + _num + " Args supported");
					}
					try {
						return (R) _mh.invoke(args);
					} catch (Throwable t) {
						throw Ex.Sneaky(t);
					}
				};
		...
		public class FnReduceArray<E> implements I.Fn<E, E, E> {
			...
			@SuppressWarnings("unchecked")
			@Override
			public Function<Object[], E> getArgN() {
				return (es) -> Arr.reduce(_f2, (E[])es);
			}
		}
		...
		public class FnReduceInit<E> implements I.Fn<E, E, E> {
			...
			@Override
			public Function<Object[], E> getArgN() {
				return (es) -> Arr.reduce(_f2, _init, (E[])es);
			}
		}
	}
	...
	public interface H {
		...
		public class Reduce<E, R> implements I.Fn<R, Iterator<E>, Object>, T.Rf<E, R> {
			...
			public Reduce(Supplier<R> init, BiFunction<R, E, R> step, Supplier<Boolean> end) {
				...
				_step = step;
				_end = end;
				_f1 = It.reduce(_init.get(), _step, _end);
			}
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static I.Fn toReduceInit(Object init, BiFunction p) {
		return new T.FnReduceInit(init, p);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static I.Fn toReduceArray(Object init, BiFunction p) {
		return new T.FnReduceArray(init, p);
	}
