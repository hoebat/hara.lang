...
public class CompilerTest extends TestCase {
    ...
    public void testCompileWithInvalidExpression() {
        ...
        try {
            ...
            assertTrue("Expected CompilerException, but got " + e.getCause(), e.getCause() instanceof CompilerException);
            assertEquals("Unsupported expression format", e.getCause().getMessage());
        }
    }

    public void testCompileWithBitwiseAnd() {
        assertCompile("(fn [x] (& x 3))", 1L);
    }

    public void testCompileWithBitwiseOr() {
        assertCompile("(fn [x] (| x 3))", 3L);
    }

    public void testCompileWithBitwiseXor() {
        assertCompile("(fn [x] (xor x 3))", 2L);
    }

    public void testCompileWithBitwiseNot() {
        assertCompile("(fn [x] (not x))", -2L);
    }
}
