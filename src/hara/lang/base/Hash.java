...
public interface Hash {
	...
	public interface Sip {
		...
		/**
		 * Hashes a data input for a given key, using the provided rounds of
		 * compression.
		 *
		 * @param key  the key to seed the hash with.
		 * @param data the input data to hash.
		 ...
		 *
		 * This is very close to {@link Long#rotateLeft(long, int)} aside from the use
		 * of the 64 bit masking.
		 *
	...
	public static long hashOrdered(Iterable xs) {
		int n = 0;
		int hash = 1;

		for (Object x : xs) {
			hash = 31 * hash + hashFast(x);
			++n;
		}
	...
	@SuppressWarnings("rawtypes")
	public static long hashUnordered(Iterable xs) {
		int hash = 0;
		int n = 0;
		for (Object x : xs) {
			hash += hashFast(x);
