...
export function activate(context: vscode.ExtensionContext) {
    ...
    let switchSessionDisposable = vscode.commands.registerCommand('hara.switchSession', async () => {
        try {
            ...
            const items = sessions.map((s: string) => ({
                label: s,
    ...
    let deleteSessionDisposable = vscode.commands.registerCommand('hara.deleteSession', async () => {
        try {
            ...
            const deletableSessions = sessions.filter((s: string) => s !== 'ROOT');
...
class HaraClient {
    ...
    private pendingRequest: { resolve: (val: string) => void, reject: (err: Error) => void } | null = null;
    ...
    constructor(output: vscode.OutputChannel, context: vscode.ExtensionContext) {
        ...
        const defaultSession = config.get<string>('session', 'ROOT');
        this.currentSession = context.workspaceState.get<string>('hara.currentSession', defaultSession);
    }
    ...
    private getConfig() {
        ...
        return {
            ...
            port: config.get<number>('server.port', 4164),
        };
    }
    ...
    public setCurrentSession(session: string): void {
    ...
    public async send(args: string[]): Promise<string> {
    ...
    private parseAny(offset: number): { result: any, consumed: number } {
        if (offset >= this.buffer.length) throw new Error("Incomplete");

        const type = String.fromCharCode(this.buffer[offset]);
        const CRLF_BUF = Buffer.from('\r\n');

        const findCrlf = (start: number) => {
        ...
        if (type === '+' || type === '-' || type === ':') {
            ...
            if (idx === -1) throw new Error("Incomplete");

            const content = this.buffer.toString('utf8', offset + 1, idx);
            const consumed = idx + 2 - offset; // +2 for CRLF

            if (type === '-') throw new Error(content);
            if (type === ':') return { result: parseInt(content), consumed };
            ...
            // Need content + CRLF
            if (this.buffer.length < contentEnd + 2) throw new Error("Incomplete");
            ...
            if (isNaN(count)) throw new Error("Invalid array length");
